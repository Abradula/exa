...
export function UsersModalCtrl(
        ...
        login.login(consoleSession.sessionUser, password, function (loggedIn) {
          if (loggedIn) {
            ...
            Alerting.AlertAddMsg(
              "top",
              "danger",
              "An error has occurred while trying to log you in again with you new credentials"
            );
            notifyUser();
          }
        });
  ...
  $scope.validateFormUsers = function () {
    ...
    // groups validation
    console.log("userGroups", $scope.ngError);
  ...
  $scope.onUserGroup = function (group) {
    const checkbox = document.getElementById(group);
    if (checkbox.checked) {
      if ($scope.user?.groups?.length) {
        $scope.user.groups.push(group);
      } else {
        $scope.user.groups = [group];
      }
    } else {
      $scope.user.groups = $scope.user.groups.filter((g) => g !== group);
    }
  };

  $scope.hasGroup = function (group) {
    return $scope.user?.groups?.includes(group);
  };
