...
export class AppService {
  ...
  async saveClaims(body: any, authHeader: string) {
    try {
      const url = config.openimis_url;
      const token = await this.getToken(authHeader);
      const headers = { Authorization: `Bearer ${token}` };
      const response = await firstValueFrom(
        this.http.post(`${url}/claim`, body, { headers }),
      );
      // TODO check on the response claim codes and throw appropriate error if needed by ZanEMR
  ...
  getResponseBody(data: any, id: string) {
    ...
    return {
      ...
      sent_id: id,
      insurance_details,
      photo_path: otherDetails['photoPath'],
      firstname: otherDetails['firstName'],
      middlename: otherDetails['middleName'],
      lastname: otherDetails['lastName'],
      othername: otherDetails['otherName'] ?? '',
      dob: otherDetails['dob'],
      sex: otherDetails['gender'],
      phone_number: otherDetails['phone'],
      country_code: otherDetails['countryCode'] ?? '',
      address: {
        region: otherDetails['region'] ?? '',
        district: otherDetails['district'],
        shehia: otherDetails['shehia'],
      },
      place_of_birth: {
        region: '',
        district: '',
